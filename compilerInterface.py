...
class EnolaCompiler(CompilerInterface):
    ...
    def _countRydbergGates(self, enolaJson: List[Dict]) -> int:
        numGates = 0
        for item in enolaJson:
            if item.get("type") == "Rydberg":
    ...
    def _calcFidelity(self, enolaJson: List[Dict]) -> float:
        ...
        numberRydbergGates = self._countRydbergGates(enolaJson)
...
class _QMapBaseCompiler(CompilerInterface):
    ...
    def _countMoveOps(self, instructions: List[str]) -> int:
        """Count move operations, including multi-line move blocks like:
        @+ move [
            (x, y) atom0
            (x, y) atom1
        ]
        ...
        for line in instructions:
            ...
            if in_move_block:
                if line == "]":
                    ...
                    if "atom" in line:
                        count += 1
            elif line.startswith("@+ move ") and "[" not in line:
                count += 1
    ...
    def _sumMovingDistance(self, instructions: List[str]) -> float:
        ...
        singleMovePattern = re.compile(r"^@\+\s+move\s+\(([-+]?\d*\.?\d+),\s*([-+]?\d*\.?\d+)\)\s+(atom\d+)\s*$")
        blockMovePattern = re.compile(r"^\(([-+]?\d*\.?\d+),\s*([-+]?\d*\.?\d+)\)\s+(atom\d+)\s*$")
        total = 0.0
        in_move_block = False
        ...
        for line in instructions:
            ...
            if in_move_block:
                mMove = blockMovePattern.match(line)
            else:
                mMove = singleMovePattern.match(line)
    ...
    def _countTrapSwaps(self, instructions: List[str]) -> int:
        """Count trap swap operations, including single-line and multi-line load/store blocks.
        Examples:
        - Single: @+ load atom0 → +1
        - Block: @+ load [ atom0, atom1 ] → +2
        """
        count = 0
        in_load_block = False
        ...
        for line in instructions:
            ...
            if in_load_block or in_store_block:
                if "atom" in line:
                    count += 1
            elif line.startswith("@+ load ") and "[" not in line:
                count += 1
            elif line.startswith("@+ store ") and "[" not in line:
                count += 1
...
class WeaverCompiler(CompilerInterface):
    ...
    def _countMoveOps(self, instructions: List[str]) -> int:
        """Count move operations, but only for rows/cols that contain qubits.
        
        Tracks qubit positions from @transfer ops, then only counts shuttles
        that affect those rows/columns (ignoring empty parallel shuttle blocks).
        """
        occupied_rows = set()
        occupied_cols = set()
        transfer_pattern = re.compile(r"^@transfer(?:\s+\d+)?\s+\((\d+)\s*,\s*(\d+)\)")
        shuttle_pattern = re.compile(r"^@shuttle\s+(row|col)\s+(\d+)")
        
        # Extract occupied rows/cols from transfer operations
        for line in instructions:
            m = transfer_pattern.match(line)
            if m:
                col, row = int(m.group(1)), int(m.group(2))
                occupied_rows.add(row)
                occupied_cols.add(col)
        
        # Count only shuttles affecting occupied rows/cols
        count = 0
        for line in instructions:
            m = shuttle_pattern.match(line)
            if m:
                direction, index = m.group(1), int(m.group(2))
                if direction == "row" and index in occupied_rows:
                    count += 1
                elif direction == "col" and index in occupied_cols:
                    count += 1
        
        return count

    def _sumMovingDistance(self, instructions: List[str]) -> float:
        """Sum shuttle distances, but only for rows/cols that contain qubits.
        ...
        occupied_cols = set()
        transfer_pattern = re.compile(r"^@transfer(?:\s+\d+)?\s+\((\d+)\s*,\s*(\d+)\)")
        shuttle_pattern = re.compile(r"^@shuttle\s+(row|col)\s+(\d+)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)$")
        ...
        # Sum distances only for shuttles affecting occupied rows/cols
        total = 0.0
        for line in instructions:
            m = shuttle_pattern.match(line)
            if m:
                direction, index, distance = m.group(1), int(m.group(2)), float(m.group(3))
                if direction == "row" and index in occupied_rows:
                    total += abs(distance)
                elif direction == "col" and index in occupied_cols:
                    total += abs(distance)
    ...
    def _countTrapSwaps(self, instructions: List[str]) -> int:
        return sum(2 for line in instructions if line.startswith("@transfer "))
