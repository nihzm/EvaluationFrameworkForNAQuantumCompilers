...
We use GitHub to [host code](https://github.com/munich-quantum-toolkit/qmap), to [track issues and feature requests][issues], as well as accept [pull requests](https://github.com/munich-quantum-toolkit/qmap/pulls).
...
- 🐛 Report bugs:
  ...
  Use the _🐛 Bug report_ template at <https://github.com/munich-quantum-toolkit/qmap/issues>.
...
- 💡 Propose features:
  ...
  Use the _✨ Feature request_ template at <https://github.com/munich-quantum-toolkit/qmap/issues>.
...
- 🙌 Community support:
  ...
  <https://github.com/munich-quantum-toolkit/qmap/discussions>.
...
:::{tip}
If you want to disable configuring and building the C++ tests, you can pass {code}`-DBUILD_MQT_QMAP_TESTS=OFF` to the CMake configure step.
:::
...
To build only the Python bindings, pass {code}`-DBUILD_MQT_QMAP_BINDINGS=ON` to the CMake configure step.
...
For the compiled parts of the code base, we provide type hints in the form of stub files in the {code}`python/mqt/qmap` directory.
...
MQT QMAP adheres to [Semantic Versioning], with the exception that minor releases may include breaking changes.
...
Use it to explain how users may need to adapt their usage of MQT QMAP, highlight new workflows, or clarify the impact of important updates.
...
When it is time to release a new version of MQT QMAP, create a PR that prepares the release.
This PR should:
...
This does not mean that the dependency update itself is a breaking change for MQT QMAP.
If you are sure that the dependency update does not introduce any breaking changes for MQT QMAP, you can remove the {code}`minor` or {code}`major` label from the PR.
...
Afterward, navigate to the [Releases page](https://github.com/munich-quantum-toolkit/qmap/releases) on GitHub, edit the release draft if necessary, and publish the release.
