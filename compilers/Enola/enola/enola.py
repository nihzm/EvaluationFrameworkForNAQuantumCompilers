...
class Enola:
    ...
    def __init__(self, name: str, dir: str | None = None, print_detail: bool = False, 
        ...
        self.result_json = {}
        self.result_json['name'] = Path(name).name
    ...
    def solve(self, save_file: bool = True):
        ...
        if save_file:
            ...
            if global_dict["full_code"]:
                with (code_dir / f"{self.result_json['name']}_code_full.json").open('w') as f:
                    json.dump(program_list, f)
                    for instruction in program_list:
                        instruction["state"] = {}
            with (code_dir / f"{self.result_json['name']}_code.json").open('w') as f:
                json.dump(program_list, f)
            with (time_dir / f"{self.result_json['name']}_time.json").open('w') as f:
