...
class NA_Architecture:
    ...
    def execute_frontier(self, full_instruction_list, frontier, layer_id):

        # Execute the gates in the frontier (currently a dummy function)
        for gate in frontier:
            if len(gate) == 1:
                (q,) = gate
                # If you actually execute something here, call it before/after logging
                ...
                # Helpful runtime context (optional)
                p1 = np.array(self.get_qubit_position(q1))
                p2 = np.array(self.get_qubit_position(q2))
                ...
                # Multi-qubit gate / unexpected encoding
                full_instruction_list.append((
                    layer_id,
                    "gate_nq_unknown",
                    tuple(gate),
    ...
    def _log_move(self, full_instruction_list, q_moved, from_pos, to_pos, peer_q, gate_index, dist):
        full_instruction_list.append((
            gate_index,
            "move",
            (q_moved,),
    ...
    full_instruction_list.append(f"LOADING qubit {q1} aod trap") #Mine - or is q2 loaded ?
                            full_instruction_list.append(f"MOVING qubit {q1} a distance of {np.linalg.norm(np.array(self.get_qubit_position(q1)) - np.array(self.get_qubit_position(q2)))} to position {self.get_qubit_position(q2)}") #Mine
    ...
    def compile_circuit(self):
        ...
        gate_lists = self.categorize_by_qubit_id(self.get_gates(self.qasm_str)) #of form {1:[[1,2],[1]]...} #Me: The Backlog of all gates to execute sorted by qubit id
        all_layers = [] #Me: The final list of layers to execute
        move_count = 0
        swap_trap_count = 0
        execute_count = 0
        total_max_moved_dist = 0
        swap_trap_gates = [] #Me: Not yet understood + not used in run_baseline.py -> i added it
        full_instruction_list = [] #Mine
        layer_i = -1 #Me: The current layer index being constructed
        ...
        while any(gate_lists[qubit_id] for qubit_id in gate_lists):
            layer_i += 1 #Mine
            curr_layer = [] #Me: The current layer/step being constructed
            movement_done = False
            
            qubit_list = list(gate_lists.keys()) 
            processed_qubits = set()
            gates_to_pop = []
            #For each qubit, get its next gate (if not already processed in this layer) + add qubits id to gates_to_pop if we have a gate to execute for the qubit
            for qubit_id in qubit_list:
                ...
                if gate_lists[qubit_id]:
                    ...
                    if len(gate_lists[qubit_id][0]) == 1:
                        gates_to_pop.append(qubit_id) #Me: Shouldn't the qubit also be added to processed_qubits here? -> And then gates_to_pop would be identical to processed_qubits?
                                                       # ==> Since we dont come back to the qubit_id it doesnt matter (We dont have a single and multiple qubit gate on same qubit in one layer) but yes they are identical i think -> confirm ?
                    ...
                    # Two qubit gate: 
                        ...
                        if gate_lists[other_qubit_id] and gate_lists[qubit_id][0] == gate_lists[other_qubit_id][0]: #Me: Does this check if neither qubit has been processed yet this layer? -> yes, first check is just so that 2. one is no seg-fault
            ...
            for i in reversed(range(len(curr_layer))):
                ...
                if len(gate) > 1: 
                    ...
                    if dist > self.radius and q1 not in self.mobile_qubits and q2 not in self.mobile_qubits:
                        move_count += 1
                        swap_trap_count += 1
                        self.trap_dist += dist
                        ...
                        curr_layer.pop(i)
                        continue
                    #If it's a multi-qubit gate but movement is done for this layer and still not in range, need to re-add the gate to the gatelist
                    elif movement_done and dist > self.radius:
                        for q in gate:
                        ...
                        if dist > self.radius: 
                            if q1 in self.mobile_qubits or q2 in self.mobile_qubits:
                                ...
                                if q1 in self.mobile_qubits:
                                    ...
                                    self._log_move(full_instruction_list, q1, self.get_qubit_position(q1), (x2,y2), q2, layer_i, dist)  # Mine
                                ...
                                #Do the same as above on qubit 2 in the gate                       
                                    ...
                                    self._log_move(full_instruction_list, q2, self.get_qubit_position(q2), (x2,y2), q1, layer_i, dist)  # Mine
            ...
            # Execute the gates in the frontier (currently a dummy function)
            self.execute_frontier(full_instruction_list, curr_layer, layer_i)

            #Reset position of moved qubit (if any)
            if self.moved_atoms_list:
                moves = []
                for atom_id, (original_x, original_y) in self.moved_atoms_list:

                    curr_x, curr_y = self.get_qubit_position(atom_id)

                    # distance for this move (return/reset)
                    dist = np.linalg.norm(
                        np.array((curr_x, curr_y)) - np.array((original_x, original_y))
                    )
                    #append move
                    moves.append(dist)

                    self._log_move(full_instruction_list, atom_id, (curr_x, curr_y), (original_x, original_y), None, layer_i, dist)  # Mine
                    ...
                    self.aod.atom_by_id[atom_id].row.position = original_y
                    self.aod.atom_by_id[atom_id].col.position = original_x
            ...
            for gate in curr_layer: #Me: Useless 
